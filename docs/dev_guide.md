...
The files generated by the `Makefile` plus the collection of `.md` files makeup the documentation available through [Github Pages](https://pages.github.com/). [Github Actions](https://github.com/features/actions) is responsible for handling updates to the documentation. Updates are automated on pushes to the `master` branch and the build status is available on the commit status pop-up.
